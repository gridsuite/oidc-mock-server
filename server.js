...
const clientConfigs = clientNums.map((clientNum) => {
  ...
  if (process.env[`CLIENT_SILENT_REDIRECT_URI${suffix}`]) {
    clientConfig.clientSilentRedirectUri =
      process.env[`CLIENT_SILENT_REDIRECT_URI${suffix}`];
  }

  clientConfig.redirect_uris = [
    clientConfig.clientRedirectUri,
    clientConfig.clientSilentRedirectUri,
  ].filter(Boolean);
...
const usersProfile =
  process.env.USERS_PROFILE || "UTILISATEURS|ADMIN|ADMIN_EXPLORE";
...
const oidcConfig = {
  async findAccount(ctx, id) {
    ...
    if (ctx.oidc.session) {
      ...
      ctx.oidc.session.promptedScopesFor(
    ...
    return {
      accountId: id,
      async claims() {
        return { sub: id, name: id, email: id + domain, profile: usersProfile };
      },
    };
  },
  claims: {
    openid: ["sub", "name", "email", "profile"],
  },
